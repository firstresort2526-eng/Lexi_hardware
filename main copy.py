...
def explain_text(cam, voice):
    ...
    explanation_chat = Chat("你係為讀寫障礙小學生而設的學習助手。用家會把鉛筆指向某一中文字，請依照文字出處判斷用家希望得到解釋的詞語，並只輸出有效JSON，請用廣東話\n輸出格式為'你指住嘅詞語係{...}， {精簡解釋...}'")
    reply = explanation_chat.send_message(prompt)
    explanation_timestamp = time.perf_counter()

    print(f"Lexi 學習助手: {reply}")
    play_explain_audio(reply)

    respond_splitted = reply.split('，')
    word = respond_splitted[0][7:]
    meaning = respond_splitted[1]
    # Upload to firebase
...
def categorize_instructions(audio_prompt):
    ...
    return categorizer.send_message(audio_prompt)
